...
import com.yhtx.forms.prop.FormsProp;
import com.yhtx.forms.prop.FormsPropDb;
...
public class EntityManagerService implements DisposableBean {
    ...
    private synchronized EntityManagerFactory getEntityManagerFactory(String dbName) {
        ...
        for (FormsPropDb prop : eruptProp.getDbs()) {
    ...
    public <R> R getEntityManager(Class<?> formsClass, Function<EntityManager, R> function) {
        EntityManager extEntityManager = getExtEntityManager(formsClass);
        if (null != extEntityManager) return function.apply(extEntityManager);
        FormsDataSource formsDataSource = formsClass.getAnnotation(FormsDataSource.class);
        if (null == formsDataSource) return function.apply(entityManager);
        EntityManager em = this.getEntityManagerFactory(formsDataSource.value()).createEntityManager();
        try {
